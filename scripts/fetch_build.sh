...
NEW_VERSION="${NEW_VERSION:?NEW_VERSION is required}"          # e.g. 6.3.0
...
# Allow overriding filenames if your repo uses custom names
TRIL_FILE="${TRIL_FILE:-TRILLIUM_5GCN_CNF_REL_${NEW_VERSION}.tar.gz}"
BIN_FILE="${BIN_FILE:-BIN_REL_${NEW_VERSION}.tar.gz}"

# If you want BIN to be mandatory, set REQUIRE_BIN_REL=true
...
REMOTE_DIR="${BUILD_SRC_BASE%/}/${NEW_VERSION}"
...
if ssh "${SSH_OPTS[@]}" "${BUILD_SRC_USER}@${BUILD_SRC_HOST}" "test -s '${REMOTE_DIR}/${BIN_FILE}'"; then
  ...
  # try to auto-discover any BIN*<ver>*.tar.gz
  BIN_CAND="$(ssh "${SSH_OPTS[@]}" "${BUILD_SRC_USER}@${BUILD_SRC_HOST}" \
    "ls -1 ${REMOTE_DIR}/BIN*${NEW_VERSION}*.tar.gz 2>/dev/null | head -n1 || true")"
  if [[ -n "${BIN_CAND}" ]]; then
...
if [[ "${HAVE_BIN}" -eq 1 ]]; then
  SRC_LIST+=("${BUILD_SRC_USER}@${BUILD_SRC_HOST}:${REMOTE_DIR}/${BIN_FILE}")
fi
